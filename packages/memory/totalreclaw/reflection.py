...
def reflect(store: MemoryStore) -> list[str]:
    """Run a deterministic, dependency-free reflection pass.

    README contract: reflect(store) — synthesizes patterns, prunes low-signal
    entries. Without an LLM in the loop, this groups recent episodes by
    goal_tag, writes a single reflection summarizing the counts, and applies
    importance decay to older reflections.
    """
    recent = store.get_recent(limit=50, memory_type="episode")
    by_goal: dict[str, int] = {}
    for m in recent:
        key = m.goal_tag or "general"
        by_goal[key] = by_goal.get(key, 0) + 1
    if not by_goal:
        mem = store.save_reflection("No new activity to reflect on.", importance=4)
        store.decay_old_reflections()
        return [mem.id]
    summary = "; ".join(f"{goal}: {count} entries" for goal, count in sorted(by_goal.items()))
    mem = store.save_reflection(f"Pattern summary — {summary}", importance=7)
    store.decay_old_reflections()
    return [mem.id]
